    ...
    def applyPython(self, spark: SparkSession, in0: DataFrame) -> DataFrame:
        ...
        for r in order_rules:
            if (r.expression.expression or "").strip():
                if r.sortType == "asc":
                    order_cols.append(expr(r.expression.expression).asc())
                elif r.sortType == "asc_nulls_last":
                    order_cols.append(expr(r.expression.expression).asc_nulls_last())
                elif r.sortType == "desc_nulls_first":
                    order_cols.append(expr(r.expression.expression).desc_nulls_first())
                else:
                    order_cols.append(expr(r.expression.expression).desc())
