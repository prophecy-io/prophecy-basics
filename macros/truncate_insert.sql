{#
  ...
  Adapter support:
    Every adapter goes through the one materialization below. The TRUNCATE
    itself is dispatched (see `prophecy_truncate_relation` at the bottom of this
    file) because BigQuery cannot use `adapter.truncate_relation()`.

  Known gap:
    A `pre_hook`/`post_hook` configured with `transaction: false` makes dbt's
    `run_hooks` emit a bare `commit;`, which BigQuery does not accept. Hooks
    left at the default `transaction: true` are unaffected.
...
{#
  ...
  dbt has no `truncate_relation` macro to override: `adapter.truncate_relation()`
  is a Python method. On every adapter built on dbt's SQLAdapter it dispatches
  onward to the `truncate_relation` macro, so the default below is a plain
  pass-through.

  BigQueryAdapter extends BaseAdapter instead, and `truncate_relation()` is an
  abstractmethod there, so BigQuery has to supply its own -- and supplies one
  that raises:

      NotImplementedError: `truncate` is not implemented for this adapter!

  Nothing ever dispatches to a `bigquery__truncate_relation` macro, so the
  override has to happen on a name of our own. The DDL itself is supported by
  BigQuery; only the adapter method is missing.
#}
...
{% macro bigquery__prophecy_truncate_relation(relation) %}
  {#-- Same SQL dbt's own `default__truncate_relation` emits. --#}
