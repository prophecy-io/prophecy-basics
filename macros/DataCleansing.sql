...
{% macro snowflake__DataCleansing(
        relation_name,
        schema,
        modifyCase,
        columnNames=[],
...
{% macro bigquery__DataCleansing(
    ...
    {% for col_name in columnNames %}
        ...
        {% if dtype == "string" %}
            ...
            {% endif %}
            {% if removeTabsLineBreaksAndDuplicateWhitespace %}
                {% set col_expr = "REGEXP_REPLACE(" ~ col_expr ~ ", r'\\s+', ' ')" %}
            {% endif %}
            {% if allWhiteSpace %}
                {% set col_expr = "REGEXP_REPLACE(" ~ col_expr ~ ", r'\\s+', '')" %}
            {% endif %}
            {% if cleanLetters %}
                {% set col_expr = "REGEXP_REPLACE(" ~ col_expr ~ ", r'[A-Za-z]+', '')" %}
            {% endif %}
            {% if cleanPunctuations %}
                {% set col_expr = "REGEXP_REPLACE(" ~ col_expr ~ ", r'[^a-zA-Z0-9\\s]', '')" %}
            {% endif %}
            {% if cleanNumbers %}
                {% set col_expr = "REGEXP_REPLACE(" ~ col_expr ~ ", r'\\d+', '')" %}
            {% endif %}
