...
{% macro default__GenerateRows(
	 	 ...
	 	 {# 1. Replace the target column reference itself (e.g., 'c' in 'add_months(payload.c, 1)')
	 	 	 with the internal column reference from the *current* iteration (e.g., gen.__gen_c) #}
	 	 {% set _loop_tmp = _loop_tmp | replace('.' ~ q_by_adapter, '.' ~ internal_col) %}
	 	 {% set _loop_tmp = _loop_tmp | replace('.' ~ backtick_col, '.' ~ internal_col) %}
	 	 {% set _loop_tmp = _loop_tmp | replace('.' ~ doubleq_col, '.' ~ internal_col) %}
	 	 {% set _loop_tmp = _loop_tmp | replace('.' ~ singleq_col, '.' ~ internal_col) %}
	 	 {% set _loop_tmp = _loop_tmp | replace('.' ~ plain_col, '.' ~ internal_col) %}

	 	 {# 2. The reference should be `gen.<internal_col>` in the recursive step, not `payload.gen.<internal_col>` #}
	 	 {% set loop_expr_replaced = _loop_tmp | replace(internal_col, 'gen.' ~ internal_col) | replace('payload.gen.', 'gen.') %}
