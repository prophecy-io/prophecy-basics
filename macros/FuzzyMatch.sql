...
{%- if mode == 'PURGE' or mode == 'MERGE' -%}
    ...
    {%- for field in matchFields -%}
        ...
        {%- if mode == 'PURGE' -%}
            {%- set select_stmt = "select CAST(" ~ quoted_record_id_col ~ " AS VARCHAR) as record_id, upper('" ~ col ~ "') as column_name, " ~ column_value_expr ~ " as column_value, '" ~ func_name ~ "' as function_name from " ~ (relation_list | join(', ')) -%}
        {%- elif mode == 'MERGE' -%}
            {%- set select_stmt = "select CAST(" ~ quoted_record_id_col ~ " AS VARCHAR) as record_id, CAST(" ~ quoted_source_id_col ~ " AS VARCHAR) as source_id, upper('" ~ col ~ "') as column_name, " ~ column_value_expr ~ " as column_value, '" ~ func_name ~ "' as function_name from " ~ (relation_list | join(', ')) -%}
        {%- endif -%}
