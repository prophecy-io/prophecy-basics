...
{%- macro duckdb__DataMasking(
    ...
    {%- elif masking_method == "hash" -%}
        {%- if masked_column_add_method == "combinedHash_substitute" -%}
            ...
            {%- do withColumn_clause.append("md5(" ~ concat_expression ~ ") AS " ~ prophecy_basics.quote_identifier(combined_hash_column_name)) -%}
        {%- else  -%}
            {% for column in column_names %}
                {%- set quoted_column = prophecy_basics.quote_identifier(column) -%}
                {%- if masked_column_add_method == "inplace_substitute" -%}
                    {%- do withColumn_clause.append("md5(" ~ quoted_column ~ ") AS " ~ prophecy_basics.quote_identifier(column)) -%}
                {%- elif prefix_suffix_opt == "Prefix" -%}
                    {%- do withColumn_clause.append("md5(" ~ quoted_column ~ ") AS " ~ prophecy_basics.quote_identifier(prefix_suffix_val ~ column)) -%}
                {%- else -%}
                    {%- do withColumn_clause.append("md5(" ~ quoted_column ~ ") AS " ~ prophecy_basics.quote_identifier(column ~ prefix_suffix_val)) -%}
                {%- endif -%}
            {% endfor %}
        {%- endif -%}
    ...
    {%- elif masking_method == "sha2" -%}
        {% for column in column_names %}
            {%- set quoted_column = prophecy_basics.quote_identifier(column) -%}
            {%- if masked_column_add_method == "inplace_substitute" -%}
                {%- do withColumn_clause.append("sha256(" ~ quoted_column ~ ") AS " ~ prophecy_basics.quote_identifier(column)) -%}
            {%- elif prefix_suffix_opt == "Prefix" -%}
                {%- do withColumn_clause.append("sha256(" ~ quoted_column ~ ") AS " ~ prophecy_basics.quote_identifier(prefix_suffix_val ~ column)) -%}
            {%- else -%}
                {%- do withColumn_clause.append("sha256(" ~ quoted_column ~ ") AS " ~ prophecy_basics.quote_identifier(column ~ prefix_suffix_val)) -%}
            {%- endif -%}
        {% endfor %}
