...
{# BigQuery: base64, unbase64, hex, unhex, encode (→ TO_HEX), decode (→ FROM_HEX). AES not supported (requires Secret Manager). #}
{%- macro bigquery__DataEncoderDecoder(
    relation_name,
    column_names,
    remaining_columns,
    enc_dec_method,
    ...
    {{ log("Applying encoding-specific column operations (BigQuery)", info=True) }}
    {% set relation_list = relation_name if relation_name is iterable and relation_name is not string else [relation_name] %}
    ...
    {%- if enc_dec_method == "base64" -%}
        {% for column in column_names %}
            {%- set quoted_column = prophecy_basics.quote_identifier(column) -%}
            {%- if change_col_name == "inplace_substitute" -%}
                {%- do withColumn_clause.append("TO_BASE64(CAST(" ~ quoted_column ~ " AS BYTES)) AS " ~ prophecy_basics.quote_identifier(column)) -%}
            {%- elif prefix_suffix_opt == "Prefix" -%}
                {%- do withColumn_clause.append("TO_BASE64(CAST(" ~ quoted_column ~ " AS BYTES)) AS " ~ prophecy_basics.quote_identifier(prefix_suffix_val ~ column)) -%}
            {%- else -%}
                {%- do withColumn_clause.append("TO_BASE64(CAST(" ~ quoted_column ~ " AS BYTES)) AS " ~ prophecy_basics.quote_identifier(column ~ prefix_suffix_val)) -%}
            {%- endif -%}
        {% endfor %}
    {%- endif -%}
    ...
    {%- if enc_dec_method == "unbase64" -%}
        {% for column in column_names %}
            {%- set quoted_column = prophecy_basics.quote_identifier(column) -%}
            {%- if change_col_name == "inplace_substitute" -%}
                {%- do withColumn_clause.append("CAST(FROM_BASE64(" ~ quoted_column ~ ") AS STRING) AS " ~ prophecy_basics.quote_identifier(column)) -%}
            {%- elif prefix_suffix_opt == "Prefix" -%}
                {%- do withColumn_clause.append("CAST(FROM_BASE64(" ~ quoted_column ~ ") AS STRING) AS " ~ prophecy_basics.quote_identifier(prefix_suffix_val ~ column)) -%}
            {%- else -%}
                {%- do withColumn_clause.append("CAST(FROM_BASE64(" ~ quoted_column ~ ") AS STRING) AS " ~ prophecy_basics.quote_identifier(column ~ prefix_suffix_val)) -%}
            {%- endif -%}
        {% endfor %}
    {%- endif -%}
    ...
    {%- if enc_dec_method == "hex" -%}
        {% for column in column_names %}
            {%- set quoted_column = prophecy_basics.quote_identifier(column) -%}
            {%- if change_col_name == "inplace_substitute" -%}
                {%- do withColumn_clause.append("TO_HEX(CAST(" ~ quoted_column ~ " AS BYTES)) AS " ~ prophecy_basics.quote_identifier(column)) -%}
            {%- elif prefix_suffix_opt == "Prefix" -%}
                {%- do withColumn_clause.append("TO_HEX(CAST(" ~ quoted_column ~ " AS BYTES)) AS " ~ prophecy_basics.quote_identifier(prefix_suffix_val ~ column)) -%}
            {%- else -%}
                {%- do withColumn_clause.append("TO_HEX(CAST(" ~ quoted_column ~ " AS BYTES)) AS " ~ prophecy_basics.quote_identifier(column ~ prefix_suffix_val)) -%}
            {%- endif -%}
        {% endfor %}
    {%- endif -%}
    ...
    {%- if enc_dec_method == "unhex" -%}
        {% for column in column_names %}
            {%- set quoted_column = prophecy_basics.quote_identifier(column) -%}
            {%- if change_col_name == "inplace_substitute" -%}
                {%- do withColumn_clause.append("CAST(FROM_HEX(" ~ quoted_column ~ ") AS STRING) AS " ~ prophecy_basics.quote_identifier(column)) -%}
            {%- elif prefix_suffix_opt == "Prefix" -%}
                {%- do withColumn_clause.append("CAST(FROM_HEX(" ~ quoted_column ~ ") AS STRING) AS " ~ prophecy_basics.quote_identifier(prefix_suffix_val ~ column)) -%}
            {%- else -%}
                {%- do withColumn_clause.append("CAST(FROM_HEX(" ~ quoted_column ~ ") AS STRING) AS " ~ prophecy_basics.quote_identifier(column ~ prefix_suffix_val)) -%}
            {%- endif -%}
        {% endfor %}
    {%- endif -%}
